...
AnomalousApp.app = c()
AnomalousApp.flow = c()
AnomalousApp.topic = c()
app = unique(TopicsFDroid$AppName)[1]
for (app in unique(TopicsFDroid$AppName)) {
  ...
  for (a.topic in unique(TopicsFDroid$TopicID)) {
  
  # extract the line MODEL from the model, with the flows for topic DT.AUT
  model = flows.for.dominant.topic.model[a.topic, ]
  ...
  #        FLOWS.AUT[P]=1 AND MODEL[P]=0   than there is an anomaly
    for(i in aut.flows){
      if (model[i] == 0) {
        ...
        AnomalousApp.topic = c(AnomalousApp.topic, TopicsFDroid$TopicPos[TopicsFDroid$AppName==aut & TopicsFDroid$TopicID==a.topic])
      }
    }
  }
  rm(i)
...
AnomalousApp = data.frame( cbind(AppName = AnomalousApp.app, PermPairID = AnomalousApp.flow, TopicPos = AnomalousApp.topic) )
unique(AnomalousApp$AppName)
rm(AnomalousApp.app, AnomalousApp.flow)

write.csv(AnomalousApp, file="app1_all_topics.csv", row.names=FALSE)
